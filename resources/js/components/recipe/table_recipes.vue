<template >
  <div class="bg-white border mx-auto p-3 rounded shadow table_recipes">
    ...
    <table class="table align-middle mb-0 bg-white text-center">
      <caption class="font-3 m-3 text-center" v-if="this.recipes.length == 0">
      ...
      <tbody>
        <tr v-for="recipe in this.recipes" :key="recipe" class="text-center">
          <td>
            <div class="d-flex mx-auto w-50 align-items-center">
              <img :src="this.w_path +'/storage/recipes/'+recipe.images_recipe[0].name
              " alt="" style="width: 45px; height: 45px" class="rounded-circle" />
              <div class="ms-3">
                <p class="fw-bold mb-1">
                  <a class="text-decoration-none text-black" :href="'/recipes/' + recipe.id">{{ recipe.name }}</a>
                </p>
              </div>
            </div>
          </td>
          ...
          <td>
            <a @click="delete_recipe(recipe.id)" href="#" class="
                  fa fa-minus-circle
                  text-danger text-decoration-none
                  m-1
                  font-3
                "></a>
            <a @click="update_recipe(recipe.id)" href="#"
